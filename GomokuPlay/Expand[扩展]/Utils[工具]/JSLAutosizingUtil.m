...
@implementation JSLAutosizingUtil

#pragma mark - 以iPhone7的屏幕为基准进行适配
CGFloat const JSL_IPHONE_AUTOSIZE_BASIC_WIDTH = 375.0f;
CGFloat const JSL_IPHONE_AUTOSIZE_BASIC_HEIGHT = 667.0f;

#pragma mark -
#pragma mark 适配字体大小
+ (CGFloat)getAutosizeFontSize:(CGFloat)fontSize
{
    JSLIPhoneAutosizeType autosizeType = [[self class] getAutosizeType];
    
    switch (autosizeType) {
        case JSLIPhoneAutosizeTypeW320H480: {
            return SCREEN_MIN_LENGTH*(fontSize/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
        case JSLIPhoneAutosizeTypeW320H568: {
            return SCREEN_MIN_LENGTH*(fontSize/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
        case JSLIPhoneAutosizeTypeW375H667: {
            return SCREEN_MIN_LENGTH*(fontSize/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
        case JSLIPhoneAutosizeTypeW414H736: {
            return fontSize;
        }
        case JSLIPhoneAutosizeTypeW375H812: {
            return fontSize;
        }
        case JSLIPhoneAutosizeTypeW414H896: {
            return fontSize;
        }
        default: {
            return fontSize;
...
+ (CGFloat)getAutosizeViewWidth:(CGFloat)width
{
    ...
    switch (autosizeType) {
        case JSLIPhoneAutosizeTypeW320H480: {
            return SCREEN_MIN_LENGTH*(width/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
        case JSLIPhoneAutosizeTypeW320H568: {
            return SCREEN_MIN_LENGTH*(width/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
...
+ (CGFloat)getAutosizeViewHeight:(CGFloat)height
{
    JSLIPhoneAutosizeType autosizeType = [[self class] getAutosizeType];
    switch (autosizeType) {
        case JSLIPhoneAutosizeTypeW320H480: {//4,4s
            return SCREEN_MAX_LENGTH*(height/JSL_IPHONE_AUTOSIZE_BASIC_HEIGHT);
        }
        case JSLIPhoneAutosizeTypeW320H568: {//5 5se
            return SCREEN_MAX_LENGTH*(height/JSL_IPHONE_AUTOSIZE_BASIC_HEIGHT);
        }
        case JSLIPhoneAutosizeTypeW375H667: {//6,7
            return SCREEN_MAX_LENGTH*(height/JSL_IPHONE_AUTOSIZE_BASIC_HEIGHT);
        }
        case JSLIPhoneAutosizeTypeW414H736: {//6,7plus
            return height;
        }
        case JSLIPhoneAutosizeTypeW375H812: {//6,7plus
            return height;
        }
        case JSLIPhoneAutosizeTypeW414H896: {
            return height;
...
+ (CGFloat)getAutosizeViewMarginScale:(CGFloat)margin
{
    ...
    switch (autosizeType) {
        case JSLIPhoneAutosizeTypeW320H480: {
            return SCREEN_MIN_LENGTH*(margin/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
        case JSLIPhoneAutosizeTypeW320H568: {
            return SCREEN_MIN_LENGTH*(margin/JSL_IPHONE_AUTOSIZE_BASIC_WIDTH);
        }
...
+ (JSLIPhoneAutosizeType)getAutosizeType
{
    ...
    if (UIInterfaceOrientationUnknown == orientation) {
        return JSLIPhoneAutosizeTypeUnKnown;
    }
    ...
    if (UIInterfaceOrientationPortrait == orientation || UIDeviceOrientationPortraitUpsideDown == orientation) {
        if (width ==  320.0f) {
            if (height == 480.0f) {
                return JSLIPhoneAutosizeTypeW320H480;
            } else {
                return JSLIPhoneAutosizeTypeW320H568;
            }
        } else if (width == 375.0f) {
            if (height == 667.0f) {
                return JSLIPhoneAutosizeTypeW375H667;
            } else {
                return JSLIPhoneAutosizeTypeW375H812;
            }
        } else if (width == 414.0f) {
            if (height == 736.0f) {
                return JSLIPhoneAutosizeTypeW414H736;
            } else {
                return JSLIPhoneAutosizeTypeW414H896;
            }
        }
    }
    // landscape
...
{
    static JSLAutosizingUtil *_singetonInstance = nil;
