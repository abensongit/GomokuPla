...
- (void)requestWillStart:(id)request {
    if (_animatingView || _animatingText) {
        dispatch_async(dispatch_get_main_queue(), ^{
            // TODO: Show Loading
            if (self->_animatingView) {
                //
                if (self->_animatingText.length > 0) {
                    [JSLProgressHUDUtil showWithStatus:self->_animatingText];
                } else {
                    [JSLProgressHUDUtil show];
                }
            } else {
                //
                if (self->_animatingText.length > 0) {
