...
@implementation JSLBaseRequest

- (instancetype)init
{
    self = [super init];
    if (self) {
        _method = JSLRequestMethodPOST;
        [self initialize];
    }
    return self;
}
...
{
    return [self initWithRequestUrl:url parameters:parameters method:JSLRequestMethodPOST];
}
...
- (void)initialize
{
    _timeout = 60.0f; // 请求超时时间
    _cacheVersion = 0; // 缓存版本号
    _cacheTimeInSeconds = 0.0f; // 缓存保存时间
...
{
    switch (_method) {
        case JSLRequestMethodGET: {
            return YTKRequestMethodGET;
        }
        case JSLRequestMethodPOST: {
            return YTKRequestMethodPOST;
        }
        case JSLRequestMethodHEAD: {
            return YTKRequestMethodHEAD;
        }
        case JSLRequestMethodPUT: {
            return YTKRequestMethodPUT;
        }
        case JSLRequestMethodDELETE: {
            return YTKRequestMethodDELETE;
        }
        case JSLRequestMethodPATCH: {
...
- (void)requestCompleteFilter
{
    ...
    // 用户提供自己的具体实现，回调成功之前的处理
    // 弹出系统错误提示信息
    /*
     NSString *message = ((NSDictionary *)self.responseJSONObject)[@"message"];
     if (![self isHideErrorMessage]) {
     [ZLLProgressHUDUtil showWithStatus:message];
     }
     */
}
...
- (void)requestFailedFilter
{
    ...
    if (![self isHideErrorMessage]) {
        ...
        [JSLProgressAlertUtil showFailWithMessage:self.error.localizedDescription toView:controller.view];
    }
}
