...
@implementation JSLNavigationBarTitleView
...
@interface JSLNavBarViewController ()
@property (nonatomic, strong) UILabel *navigationBarTitleLabel; // 导航栏标题
@end

@implementation JSLNavBarViewController
...
- (JSLNavBarButtonItemType)prefersNavigationBarLeftButtonItemType
{
    ...
    return JSLNavBarButtonItemTypeNone;
}
...
{
    JSLNavBarButtonItemType barButtonItemType = [self prefersNavigationBarLeftButtonItemType];
    
    switch (barButtonItemType) {
            // 无按钮
        case JSLNavBarButtonItemTypeNone: {
            
            break;
        }
            // 默认返回
        case JSLNavBarButtonItemTypeDefault: {
            return ICON_NAVIGATION_BAR_BUTTON_BACK_ARROW_DEF;
            break;
        }
            // 自定义
        case JSLNavBarButtonItemTypeCustom: {
...
{
    JSLNavBarButtonItemType barButtonItemType = [self prefersNavigationBarRightButtonItemType];
                                                   ...
                                                   action:(SEL)action
{
    ...
    // 默认显示图片
    if (iconNameNormal) {
        CGFloat imageSizeWidth = JSL_AUTOSIZING_WIDTH(12.0f);
        CGFloat imageSizeHeight = JSL_AUTOSIZING_HEIGTH(20.0f);
    ...
    if (title.length > 0) {
        ...
        [btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
        // 调整控件大小
        CGFloat maxWidth = MAX(MARGIN*2.5+titleWidth, JSL_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_MAX_WIDTH));
                                                  ...
                                                  action:(SEL)action
{
    ...
    // 自定义按钮
    CGFloat imageSize = JSL_AUTOSIZING_WIDTH(20.0f);
    ...
    [btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
    [btn addTarget:self action:action forControlEvents:UIControlEventTouchUpInside];
    if (![JSLSysUtil validateStringEmpty:iconNameNormal] && [JSLSysUtil validateStringEmpty:iconNameSelect]) {
...
- (void)setupNavigationBarViewDefault
{
        ...
        if (self.navigationBarLeftButtonItem) {
            ...
            if([[NSObject OSVersion] floatValue] >= 11.0) {
                ...
                UIBarButtonItem *spacerItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace
                                                                                            target:nil
                                                                                            action:nil];
                spacerItem.width = -JSL_AUTOSIZING_WIDTH(NAVIGATION_BAR_SCREEN_MARGIN*2.0);
    ...
    // 标题区域
    {
        ...
        UIView *titleContainerView = [[JSLNavigationBarTitleView alloc] initWithFrame:titleRect];
