...
//  JSLProgressView
//
//  Created by HANSEN on 2018/10/30.
//  Copyright © 2018年 HANSEN. All rights reserved.
//

#import "JSLProgressView.h"

#define JSLProgressKWidth  self.frame.size.width
#define JSLProgressKHeight self.frame.size.height

#pragma mark - JSLProgressLayer

@interface JSLProgressLayer : CALayer
...
@implementation JSLProgressLayer {
...
- (void)setProgressWithAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        self.imageLayer.frame = CGRectMake(0, 0, _progress * JSLProgressKWidth, JSLProgressKHeight);
...
- (void)drawInContext:(CGContextRef)ctx {
    CGRect rect = CGRectMake(0, 0, self.progress * JSLProgressKWidth, JSLProgressKHeight);
    if (self.trackImage) {   // trackImage
        CGContextScaleCTM(ctx, 1, -1);
        CGContextTranslateCTM(ctx, 0, -rect.size.height);
        CGContextDrawImage(ctx, CGRectMake(0, 0, JSLProgressKWidth, JSLProgressKHeight), self.trackImage.CGImage);
    }
...
- (void)layoutSublayers {
    ...
    self.textLayer.frame = (CGRect){0, (JSLProgressKHeight - size.height) / 2, JSLProgressKWidth, size.height};
...
- (void)setRoundedCorner:(NSNumber *)roundedCorner {
    ...
    self.cornerRadius = JSLProgressKHeight / 2.0;
    self.imageLayer.cornerRadius = JSLProgressKHeight / 2.0;
...
- (CALayer *)imageLayer {
    if (!_imageLayer) {
        ...
        _imageLayer.frame = CGRectMake(0, 0, self.progress * JSLProgressKWidth, JSLProgressKHeight);
...
#pragma mark - JSLProgressView

@implementation JSLProgressView

+ (Class)layerClass {
    return [JSLProgressLayer class];
}
...
- (void)setProgress:(float)progress animated:(BOOL)animated {
    ...
    JSLProgressLayer *layer = (JSLProgressLayer *)self.layer;
    layer.progress = _progress;
    [(JSLProgressLayer *)self.layer setProgressWithAnimated:animated];
}
