...
#import "YHSDropDownMenuCell.h"

//default value
#define YHSDefaultFloat -10.0
#define YHSDefaultCell @"YHSDropDownMenuCell"
#define YHSDefaultMenuScaleType YHSDropDownMenuViewAnimateType_ScaleBasedTopRight

//rgb color  r-red 、  g-green  、  b-blue  、 a-alpha
#define YHSDropDownMenuColor(r, g, b, a) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:(a)]

/**
 *  菜单展现的动画类型
 */
typedef NS_ENUM(NSInteger, YHSDropDownMenuViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    YHSDropDownMenuViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    YHSDropDownMenuViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    YHSDropDownMenuViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    YHSDropDownMenuViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    YHSDropDownMenuViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    YHSDropDownMenuViewAnimateType_FallFromTop,
};


@protocol YHSDropDownMenuViewDelegate <NSObject>

@optional

/** 若是自定义cell样式的，可以在这个代理方法中稍微小修改cell的样式，比如是否需要下划线、字体的颜色等等*/
/** you can modify menu cell style, Such as if should show underline */
- (void)yhsDropDownMenuView:(YHSDropDownMenuView *)menuView WillAppearMenuCell:(YHSDropDownMenuBasedCell *)menuCell index:(NSInteger)index;

- (void)yhsDropDownMenuViewWillAppear;

- (void)yhsDropDownMenuViewWDidAppear;

- (void)yhsDropDownMenuViewWillDisappear;

- (void)yhsDropDownMenuViewWDidDisappear;
 ...
 *  下拉菜单
 */
@interface YHSDropDownMenuView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(YHSDropDownMenuModel模型对象)
 *  @param menuWidth           菜单的宽度                ---- 若要使用默认宽度、传YHSDefaultFloat
 *  @param eachItemHeight      每一个选项的高度           ---- 若要使用默认高度、传YHSDefaultFloat
 *  @param menuRightMargin     菜单条离屏幕右边的间距      ---- 若要使用默认间距、传YHSDefaultFloat
 *  @param triangleRightMargin 三角形距离屏幕右边的间距    ---- 若要使用默认间距、传YHSDefaultFloat
 *
 *  @return 实例化的一个下拉菜单对象
 */
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage YHSDropDownMenuModel instances
 *  @param menuWidth           drop-down menu's width                --- if want to use default width , pass YHSDefaultFloat
 *  @param eachItemHeight      each item height                      --- if want to use default height, pass YHSDefaultFloat
 *  @param menuRightMargin     drop-down menu from the right margin  --- if want to use default margin, pass YHSDefaultFloat
 *  @param triangleRightMargin triangle from the right margin        --- if want to use default margin, pass YHSDefaultFloat
 *
...
///   >>步骤:
///   1、 [YHSDropDownMenuView alloc] init]
///   2、 对需要赋值的属性进行赋值
///       >>>>>若要使用默认值，可以不对属性进行赋值，或者赋值 YHSDefault....
///
///   3、 调用 setup方法
///==================================================================
...
///       >>>>>if want to use the default value，not assign or assign YHSDefault....
///
///   3、 call setup method
///==================================================================
...
/** 6、默认菜单样式 的分割线颜色(默认为白色)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     default menu style: separaterLineColor (if not assign, default value is white color)
 */
...
/** 1、下拉菜单模型数组(数组里面存放 YHSDropDownMenuBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage YHSDropDownMenuModel instances or the YHSDropDownMenuModel's subclass instances )
 */
@property (nonatomic, strong) NSArray *menuModelsArray;

/** 2、cell的类名, 必须是YHSDropDownMenuBasedCell的子类
 *               若用框架自带的cell,直接传@"YHSDropDownMenuCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"YHSDropDownMenuCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of YHSDropDownMenuBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"YHSDropDownMenuCell"
...
/** 7、菜单选项的背景颜色(若不设置，默认为白色、可以用YHSColor(r, g, b, a) 设置带透明度的颜色) 
 ...
 *            you can use YHSColor(r, g, b, a) to set a color with alpha.
...
/** 8、三角形颜色(若不设置，默认为白色、可以用YHSColor(r, g, b, a) 设置带透明度的颜色) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
...
/** 15、菜单的弹出的动画效果类型(若不设置，默认为YHSDropDownMenuViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is YHSDropDownMenuViewAnimateType_ScaleBasedTopRight)
 */
@property (nonatomic, assign) YHSDropDownMenuViewAnimateType menuAnimateType;
 ...
 *
 */
@property (nonatomic, weak) id<YHSDropDownMenuViewDelegate> delegate;
