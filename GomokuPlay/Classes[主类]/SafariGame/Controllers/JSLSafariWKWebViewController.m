...
#import "JSLAutosizingUtil.h"
#import "JSLSysCoreMacro.h"
#import "JSLAssetsMacro.h"
#import "JSLSysConst.h"
#import "JSLSysUtil.h"
#import "UIColor+Extension.h"
#import "UIImage+Resize.h"
#import "UIView+Frame.h"

@interface JSLSafariWKWebViewController () <WKUIDelegate, WKNavigationDelegate>
@property (nonatomic, assign) BOOL isLocalHTMLString; // 默认NO
@property (nonatomic, strong) YHSDropDownMenuView *dropdownMenu;
@end

@implementation JSLSafariWKWebViewController
...
- (instancetype)initWithHTMLUrlString:(NSString *)htmlUrlString
{
    self = [super init];
    if (self) {
        _htmlUrlString = [JSLSysUtil stringByTrimmingWhitespaceAndNewline:htmlUrlString];
...
- (void)viewDidLoad
{
    ...
    // 1.导航栏右侧按钮
    {
        ...
        // 缓存
        YHSDropDownMenuModel *menuModel4 = [YHSDropDownMenuModel yhs_DropDownMenuModelWithMenuItemTitle:@"清除缓存" menuItemIconName:nil menuBlock:^{
            [weakSelf pressButtonWebViewCacheAction];
        }];
        
        // 下拉菜单
        self.dropdownMenu = [YHSDropDownMenuView yhs_DefaultStyleDropDownMenuWithMenuModelsArray:@[menuModel0, menuModel1, menuModel2, menuModel3, menuModel4] menuWidth:YHSDefaultFloat eachItemHeight:YHSDefaultFloat menuRightMargin:YHSDefaultFloat triangleRightMargin:YHSDefaultFloat];
        ...
        self.dropdownMenu.triangleRightMargin = (JSL_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_MAX_WIDTH)-18)/2.0;
    ...
    {
        // 初始化progressView
        self.progressView = [[JSLProgressView alloc] initWithFrame:CGRectMake(0, STATUS_BAR_HEIGHT, SCREEN_WIDTH, 2.0)];
        // 设置未走过进度的进度条颜色
...
- (void)createNavigationBarCloseButton
{
    ...
    CGFloat imageSize = JSL_AUTOSIZING_WIDTH(19.0f);
    ...
    closeButtonImageView.image = [UIImage imageNamed:ICON_NAVIGATION_BAR_BUTTON_CLOSE_WHITE];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"title"]) {
        ...
        JSLLog(@"加载进度 => %f", self.webView.estimatedProgress);
...
{
    JSLLog(@"响应的内容到达主页面的时候响应，刚准备开始渲染页面应用");
...
{
    JSLLog(@"在发送请求之前，决定是否跳转:%@", navigationAction.request.URL);
...
{
    // 自定义按钮
    CGFloat imageSizeWith = JSL_AUTOSIZING_WIDTH(21.0f);
    CGFloat imageSizeHeight = JSL_AUTOSIZING_WIDTH(5.6f);
    CGRect btnFrame = CGRectMake(0, 0, JSL_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_MAX_WIDTH), NAVIGATION_BAR_HEIGHT);
...
{
    return ICON_NAVIGATION_BAR_BUTTON_MORE_DEF;
}
...
{
    if (!_webView) {
        _webView = [[JSLWKWebView alloc] initWithFrame:CGRectZero configuration:self.config];
...
- (NSString *)getCurrentCookieWithDomain:(NSString *)domainString
{
    ...
    for (NSHTTPCookie*cookie in [cookieJar cookies]) {
        JSLLog(@"第一次：name=%@;value=%@", cookie.name, cookie.value);
...
- (void)setCookieWithWebView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
    ...
    for (NSHTTPCookie *cookie in cookieStorage.cookies) {
        // JSLLog(@"name=%@;value=%@", cookie.name, cookie.value);
        NSString *excuteJSString = [NSString stringWithFormat:@"setCookie('%@', '%@', 1);", cookie.name, cookie.value];
        [JSCookieString appendString:excuteJSString];
    }
    // JSLLog(@"JSCookieString=%@", JSCookieString);
    // 执行JS
